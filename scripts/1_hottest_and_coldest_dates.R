...
weather2 <- weather |>
  dplyr::mutate(
    ...
    snow_depth = as.numeric(stringr::str_replace(snow_depth_T,
                                                 "T",
                                                 "0.01"
                                                 )
                            ),
  ...
  dplyr::mutate(date2_lag = date2 - lubridate::days(1),
...
# and 3 days after
weather3 <- weather2 |>
  dplyr::inner_join(y = weather_lag,
                    by = dplyr::join_by(date2_lag == date2)) |>
  dplyr::inner_join(y = weather_lag2,
                    by = dplyr::join_by(date2_lag2 == date2)) |>
  dplyr::inner_join(y = weather_lag3,
                    by = dplyr::join_by(date2_lag3 == date2)) |>
  dplyr::inner_join(y = weather_lead,
                    by = dplyr::join_by(date2_lead == date2)) |>
  dplyr::inner_join(y = weather_lead2,
                    by = dplyr::join_by(date2_lead2 == date2)) |>
  dplyr::inner_join(y = weather_lead3,
...
weather4 <- weather3 |>
  dplyr::summarize(max_temps_3d = list(c(max_temp_lag,
                                         max_temp,
                                         max_temp_lead)),
                   max_temps_5d = list(c(max_temp_lag2,
                                         max_temp_lag,
                                         max_temp,
                                         max_temp_lead,
                                         max_temp_lead2)),
                   max_temps_7d = list(c(max_temp_lag3,
                                         max_temp_lag2,
                                         max_temp_lag,
                                         max_temp,
                                         max_temp_lead,
  ...
  dplyr::mutate(med_max_temps_3d = median(unlist(max_temps_3d)),
